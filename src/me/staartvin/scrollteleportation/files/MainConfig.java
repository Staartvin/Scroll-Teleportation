...
public class MainConfig {
    ...
    public void loadConfiguration() {
        ...
        config.options()
                .header("Scroll Teleportation v"
                        ...
                        + "\nUses is the amount of uses a scroll have before it becomes thin air. To create scrolls " +
                        "with infinite uses, just set it to -1."
                        + "\nEffects are effects that are played when the scroll is used. (The number is the duration" +
                        " in seconds)"
                        + "\nThis is a list of effects you can use: http://jd.bukkit" +
                        ".org/rb/doxygen/d3/d70/classorg_1_1bukkit_1_1potion_1_1PotionEffectType.html"
                        + "\n\nDestinations can be defined in multiple ways:"
                        + "\n	1. Fixed point (eg: 'world,x,y,z')"
                        + "\n	2. Random point (eg: 'random world')"
                        + "\n		This takes a world to get random coordinates on. If you leave the world out, a " +
                        "random world will be selected"
                        + "\n 		Be careful with this one. This can cause lag as it will try to teleport a player" +
                        " to unloaded chunks."
        ...
        // Load messages into config file
        for (LanguageString string : LanguageString.values()) {
            config.addDefault("Messages." + string.getConfigPath(), string.getDefaultString());
        }
    ...
    public String getTranslatableMessage(LanguageString message) {
        return addColourCode(config.getString(message.getConfigPath(), message.getDefaultString()));
    }
